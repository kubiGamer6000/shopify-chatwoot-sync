...
2. **Conversation history** — a detailed recap of all their support threads: problems raised, requests, actions taken by agents, promises made, and resolutions. Internal private notes / AI drafts are ignored — only real customer messages and sent agent replies are summarized.

It is generated by [`src/services/customerSummary.ts`](src/services/customerSummary.ts), which gathers the full message thread of each of the contact's conversations, builds a prompt, and asks Claude to return structured JSON (`{ overview, history }`).
...
│   ├── firestore.ts              # Lazy firebase-admin init (AI summary storage)
...
│   ├── components/          # ProfileHeader, SummaryRow, CustomerSummary, OrdersList, SubscriptionsPanel, ui/
